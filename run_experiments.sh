...
if [ $# -ne 5 ]; then
    echo "Usage: $0 <FastTrack[0|1]> <TurboMap[0|1]> <JacobiGPU[0|1]> <version> <num_iterations>"
    exit 1
fi
...
for i in $(seq 0 $(expr $num_itr - 1)); do
    for dataset in "${datasets[@]}"; do
        echo -e "[bash:] -> ./run_script.sh $dataset $FastTrack_on $TurboMap_on $JacobiGPU_on 1 $version.$i"
