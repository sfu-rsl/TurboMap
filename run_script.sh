...
if [ $# -lt 6 ]; then
    echo "Usage: $0 <dataset_name> <FastTrack[0|1]> <TurboMap[0|1]> <JacobiGPU[0|1]> <output[0=stdout|1=file]> <version> [kernel_status1] [kernel_status2]"
    exit 1
fi
...
if [ "$JacobiGPU_on" -eq 1 ]; then
    system_name+="${system_name:+&}JacobiGPU"
fi
...
if [ "$save_ostream" -eq 0 ]; then
    if $found_in_euroc; then
        cd Examples/
        ./euroc_eval_examples.sh "$FastTrack_on" "$TurboMap_on" "$JacobiGPU_on" "$kernel_status_FT" "$kernel_status_TM" "$dataset_name" "../$statsDir" 
    elif $found_in_tumvi; then
        cd Examples/
        ./tum_vi_eval_examples.sh "$FastTrack_on" "$TurboMap_on" "$JacobiGPU_on" "$kernel_status_FT" "$kernel_status_TM" "$dataset_name" "../$statsDir" 
    else
    ...
    if $found_in_euroc; then
        cd Examples/
        ./euroc_eval_examples.sh "$FastTrack_on" "$TurboMap_on" "$JacobiGPU_on" "$kernel_status_FT" "$kernel_status_TM" "$dataset_name" "../$statsDir" > "../${statsDir}/ostream.txt" 
    elif $found_in_tumvi; then
        cd Examples/
        ./tum_vi_eval_examples.sh "$FastTrack_on" "$TurboMap_on" "$JacobiGPU_on" "$kernel_status_FT" "$kernel_status_TM" "$dataset_name" "../$statsDir" > "../${statsDir}/ostream.txt" 
    else
        echo "Invalid dataset: $dataset_name"
        exit 1
    fi
