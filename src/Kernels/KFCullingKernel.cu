...
void KFCullingKernel::launch(vector<ORB_SLAM3::KeyFrame*> vpLocalKeyFrames, int* h_nMPs, int* h_nRedundantObservations) {
    ...
    int thObs = 3;
    int blockSize = 1024;
    int numBlocks = vpLocalKeyFrames.size();
    // printf("======================GPU========================\n");
    keyframeCullingKernel<<<numBlocks, blockSize>>>(d_keyframes, vpLocalKeyFrames.size(), thObs, CudaUtils::isMonocular, d_nMPs, d_nRedundantObservations);
    checkCudaError(cudaDeviceSynchronize(), "[KFCullingKernel:] Kernel launch failed");  
    // cout << endl;
    
    // printf("======================CPU========================\n");
    // KFCullingOrig(vpLocalKeyFrames, thObs, CudaUtils::isMonocular);
    // cout << endl;

    checkCudaError(cudaMemcpy(h_nMPs, d_nMPs, vpLocalKeyFrames.size() * sizeof(int), cudaMemcpyDeviceToHost), "[KFCullingKernel::] Failed to copy d_nMPs to h_nMPs");
    checkCudaError(cudaMemcpy(h_nRedundantObservations, d_nRedundantObservations, vpLocalKeyFrames.size() * sizeof(int), cudaMemcpyDeviceToHost), "[KFCullingKernel::] Failed to copy d_nRedundantObservations to h_nRedundantObservations");
} 
...
__global__ void slowKeyframeCullingKernel(MAPPING_DATA_WRAPPER::CudaKeyFrame** d_keyframes, int numKeyframes, int thObs, bool mbMonocular,
                                    int* d_nMPs, int* d_nRedundantObservations) {
